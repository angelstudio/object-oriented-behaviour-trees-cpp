...
basictimers_timer_id basictimers_create_timer( long milliseconds ) {
	basictimers_timer_id timer = (basictimers_timer_id)malloc( sizeof(basic_timer_t) );

	timer->milliseconds = milliseconds;
	basictimers_reset_timer( timer );
...
void basictimers_reset_timer( basictimers_timer_id timer ) {
	timer->reset_time = basictimers_get_current_time();
}
...
int basictimers_timer_expired( basictimers_timer_id timer ) {
	double current_time = basictimers_get_current_time();
...
LARGE_INTEGER basictimers_getFILETIMEoffset() {
	SYSTEMTIME s;
	FILETIME f;
	LARGE_INTEGER t;
...
int basictimers_clock_gettime( int X, struct timeval *tv ) {
	LARGE_INTEGER           t;
	FILETIME            f;
	double                  microseconds;
	...
	if ( !initialized ) {
		...
		if ( usePerformanceCounter ) {
			...
			offset = basictimers_getFILETIMEoffset();
...
double basictimers_get_current_time() {
	...
	basictimers_clock_gettime( 0, &timeval );
